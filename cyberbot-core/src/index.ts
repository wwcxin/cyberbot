...
const dailyRotateFileTransport = new winston.transports.DailyRotateFile({
    ...
    maxSize: config.logger.maxSize,  // 使用配置中的大小限制，默认为10MB
    maxFiles: `${config.logger.maxDays}d`,  // 使用配置中的保留天数，默认为7天
...
export const log = winston.createLogger({
    level: config.logger.level, // 使用配置中的日志级别
    transports: [
        new (winston.transports.Console)({
            format: winston.format.combine(winston.format.colorize(), alignColorsAndTime)
        }),
        dailyRotateFileTransport  // 使用日志滚动策略替代原来的文件输出
    ],
});
    ...
    try {
        const logFiles = readdirSync(logPath)
            ...
            .map(file => ({
                ...
                // 从文件名中提取日期（格式假设为YYYY-MM-DD.log）
                date: new Date(file.replace('.log', ''))
            }))
...
export class PluginManager {
    ...
    async init() {
        ...
        log.info(`[+]已启用插件: ${enabledCount}/${totalPlugins}`);
