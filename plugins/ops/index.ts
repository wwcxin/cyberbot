    ...
    setup(ctx) {
        ...
        ctx.handle("message.group", async (e) => {
            ...
            const getMemberWithCheck = async (needGroupPower = true, needOwner = false) => {
                ...
                const member = await ctx.getGroupMemberInfo(e.group_id, MentionedUserId, false);
                return member;
            };
            ...
            if (text.startsWith('#改个性签名')) {
                ...
                await ctx.bot.set_self_longnick({
                    longNick: signature
                });
            ...
            if (text === '#退群') {
                if (!hasRight) return;
                await e.reply('done', true);
                await ctx.quitGroup(e.group_id, false);
            }
            ...
            if (text.startsWith('#我要头衔')) {
                const botHasGroupPower = await ctx.isGroupOwner(e);
            ...
            if (text.startsWith('#踢')) {
                ...
                if (!member || !(ctx.isMaster(e)) || !MentionedUserId) return;
            ...
            //     await ctx.bot.set_friend_add_request({
            ...
            if (text.startsWith('#改群头像')) {
                ...
                await ctx.bot.set_group_portrait({
                    group_id: e.group_id,
                    file: imgUrl
                });
                await e.reply('done', true);
            ...
            if (text === '#开全员禁言') {
                if (!await checkPermission(true)) return; // 需要管理员权限
                await ctx.bot.set_group_whole_ban({
                    group_id: e.group_id,
                    enable: true
                });
                await e.reply('done', true);
            ...
            if (text === '#发公告') {
                ...
                await ctx.bot._send_group_notice({
                    group_id: e.group_id,
                    content: content
                });
                await e.reply('done', true);
            ...
            //         await ctx.bot.send('set_group_anonymous', {
            //             group_id: e.group_id,
            //             enable: true
            //         });
            //         await e.reply('done', true);
