...
const capture_url = 'http://localhost:9000/generate-image-from-url' // 截图接口地址
export default definePlugin({
  ...
  setup: (ctx) => {
    ...
    if (e.raw_message.includes('#截图') || e.raw_message.includes('#电脑截图')) {
...
const handleScreenshotRequest = async (e: CyberMessageEvent, ctx: CyberPluginContext, url: string, width: number, height: number): Promise<void> => {
  ...
  try {
      const base64 = await Promise.race([
          http.post(capture_url, { url, viewport: { width, height } }),
