...
import {definePlugin,Structs,NCWebsocket,AllHandlers} from "../../src/index.js";
    ...
    "插件": {
        subcommands: {
            "列表": async (ctx, e) => {
                ...
                const loadedPlugins = new Set(Array.from(plugins.keys()));
                ...
                // 显示未加载的插件
                const unloadedPlugins = allPluginsInDir.filter(name => !loadedPlugins.has(name));
                if (unloadedPlugins.length > 0) {
                    msg += "\n📦 未加载的插件:\n";
                    unloadedPlugins.forEach(name => {
                        msg += `⚪ ${name}\n`;
                    });
                }
